...
export const makeChatsSocket = (config: SocketConfig) => {
	...
	const appStateSyncTimeout = debouncedTimeout(APP_STATE_SYNC_TIMEOUT_MS, async () => {
	...
	const updateProfilePicture = async (jid: string, content: WAMediaUpload, type: string | undefined) => {
		const { img } = await generateProfilePicture(content, type);
		await query({
			tag: "iq",
			attrs: {
	...
	const newAppStateChunkHandler = (recvChats: InitialReceivedChatsState | undefined) => {
		return {
			onMutation(mutation: ChatMutation) {
				processSyncAction(mutation, ev, authState.creds.me!, recvChats ? { recvChats, accountSettings: authState.creds.accountSettings } : undefined, logger);
			},
		};
	};
	...
	const resyncMainAppState = async (ctx?: InitialReceivedChatsState) => {
		...
		await processingMutex.mutex(() => resyncAppState(ALL_WA_PATCH_NAMES, ctx)).catch((err) => onUnexpectedError(err, "main app sync"));
	};
	...
	const appPatch = async (patchCreate: WAPatchCreate) => {
		...
		if (config.emitOwnEvents) {
			...
			await decodePatches(name, [{ ...encodeResult!.patch, version: { version: encodeResult!.state.version } }], initial!, getAppStateSyncKey, onMutation, undefined, logger);
		}
	};
	...
	const fireInitQueries = async () => {
		await Promise.all([fetchAbt(), fetchProps(), fetchBlocklist(), fetchPrivacySettings(), sendPresenceUpdate(markOnlineOnConnect ? "available" : "unavailable")]);
	};
	...
	ev.on("connection.update", ({ connection }) => {
		if (connection === "open") {
			fireInitQueries().catch((error) => onUnexpectedError(error, "connection open requests"));
		}
	});
