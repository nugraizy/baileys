 ...
 */
export const makeSocket = ({ waWebSocketUrl, connectTimeoutMs, logger, agent, keepAliveIntervalMs, version, browser, auth: authState, printQRInTerminal, defaultQueryTimeoutMs, syncFullHistory, transactionOpts }: SocketConfig) => {
	...
	const onUnexpectedError = (error: Error, msg: string) => {
		logger.error({ trace: error.stack, output: (error as any).output }, `unexpected error in '${msg}'`)
	}
