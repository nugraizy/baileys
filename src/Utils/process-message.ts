...
const processMessage = async (message: proto.IWebMessageInfo, { shouldProcessHistoryMsg, ev, creds, keyStore, logger, options }: ProcessMessageContext) => {
	...
	if (protocolMsg) {
		...
		const emitParticipantsUpdate = (action: ParticipantAction) => ev.emit('group-participants.update', { id: jid, participants, action })
