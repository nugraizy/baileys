...
const makeChatsSocket = (config: LegacySocketConfig) => {
	...
	const executeChatModification = (node: BinaryNode) => {
		...
		switch (updateType) {
			...
			case "clear":
				if (node.content) {
					const ids = (node.content as BinaryNode[]).map(({ attrs }) => attrs.index);
			...
			case "unstar":
				...
				const updates: WAMessageUpdate[] = (node.content as BinaryNode[]).map(({ attrs }) => ({
					key: {
						remoteJid: jid,
						id: attrs.index,
			...
			await Promise.all([
				...
				sendNode({
					json: {
						...
						content: [{ tag: "presence", attrs: { type: "available" } }],
					},
	...
	return {
		 ...
		 * */
		presenceSubscribe: async (jid: string) => sendNode({ json: ["action", "presence", "subscribe", jid] }),
		 ...
		 * @param img
		 */
		async updateProfilePicture(jid: string, imgBuffer: Buffer, type: string | undefined) {
			jid = jidNormalizedUser(jid);

			const { img } = await generateProfilePicture(imgBuffer, type);
		...
		getBusinessProfile: async (jid: string) => {
			jid = jidNormalizedUser(jid);
			const {
				profiles: [{ profile, wid }],
			} = await query({
				json: ["query", "businessProfile", [{ wid: jid.replace("@s.whatsapp.net", "@c.us") }], 84],
				expect200: true,
				requiresPhoneConnection: false,
			});
